...
import { getAuthorData, getLatestPostsByAuthor } from "@/components/api/api.service";
...
export default function PostPage() {
	...
	const authorData = async () => {
		const fetchedData = await getAuthorData(router.query.slug as string);

		setData(fetchedData);
		setPageUrl(fetchedData.author.attributes.url);
		setIsLoading(false);
	}

	const updateAticlesList = async () => {
		try {
			const fetchedData = await getLatestPostsByAuthor(router.query.slug as string, page!);
  ...
  return(
		<>
			{data && (
				<NextSeo
					title={data?.author.attributes.seo.metaTitle}
					description={data?.author.attributes.seo.metaDescription}
					canonical={data?.author.attributes.seo.canonicalURL}
				/>
			)}
			{isLoading && (
				<Flex
			...
			{data && (
				...
				>
					<Heading
						...
						position="relative"
					>
						Published by {data.author.attributes.name}
						<Text
							as="span"
							bgColor="blue.500"
							width="20px"
						...
						>
							...
							>
								<Image
									fill
									placeholder="blur"
									blurDataURL={data.author.attributes.img.data.attributes.url}
									style={{ objectFit:"cover" }}
									sizes="(max-width: 767px) 128px, 128px"
									src={data.author.attributes.img.data.attributes.url}
									alt={data.author.attributes.name}/>
							</Box>
							<Box
							...
							>
								...
								<Text
									...
									lineHeight="24px"
								>
									{data.author.attributes.jobTitle}
								</Text>
								<Text
									fontWeight="400"
									fontSize="16px"
									lineHeight="24px"
								>
									{data.author.attributes.description}
								</Text>
							</Box>
							<Box minWidth="128px">
								<Text
								...
								{(data.author.attributes.Expertise).map((item: { text: string}, index: number) =>
									<Text
										key={index}
										fontWeight="400"
										fontSize="16px"
							...
							<Box minWidth="128px">
								...
								{data.author.attributes.instagram && (
									<ChakraLink
										as={Link}
										display="block"
										fontWeight="400"
										...
										color="gray.600"
										target="_blank"
										href={data.author.attributes.instagram}
									>
								...
								{data.author.attributes.twitter && (
									<ChakraLink
										display="block"
										as={Link}
										fontWeight="400"
										...
										color="gray.600"
										target="_blank"
										href={data.author.attributes.twitter}
									>
										<svg style={{ marginRight: "4px", display: "inline-block", position: "relative", top: "5px" }} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
								...
								{data.author.attributes.website && (
									<ChakraLink
										display="block"
										as={Link}
										fontWeight="400"
										...
										color="gray.600"
										target="_blank"
										href={data.author.attributes.website}
									>
										<svg style={{ marginRight: "4px", display: "inline-block", position: "relative", top: "5px" }} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
