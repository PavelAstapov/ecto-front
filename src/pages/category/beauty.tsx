...
import { BEAUTY_PAGE, LATESTS_POSTS_BY_CATEGORY } from "@/graphql/queries";
...
export default function PostPage(props: any) {
  ...
  return(
		<>
			...
			<NextSeo
				title={props.beauty.data.attributes.seo.metaTitle}
				description={props.beauty.data.attributes.seo.metaDescription}
				canonical={props.beauty.data.attributes.seo.canonicalURL}
			/>
			<Box
				maxWidth="1216px"
				margin="0 auto"
			...
			>
				...
				position="relative"
			>
				{props.beauty.data.attributes.title}
				<Text
					as="span"
					bgColor="blue.500"
					width="20px"
			...
			<Text
				...
				lineHeight="28px"
			>
				{props.beauty.data.attributes.description}
			</Text>
			</Box>
			<Flex
				maxWidth="1216px"
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const client = new ApolloClient({
	...
	const latestPosts = await client.query({
		...
		variables: { category: data.beauty.data.attributes.title,  page: (context.query?.page && +(context.query?.page)) || 1 },
	})
