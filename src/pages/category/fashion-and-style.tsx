...
import { FASHION_PAGE, LATESTS_POSTS_BY_CATEGORY } from "@/graphql/queries";
...
export default function PostPage(props: any) {
	...
	useEffect(() => {
		...
		if(pageUrl !== undefined){
			if(page !== 1) {
				...
				`/category/fashion-and-style?page=${page}`,
				{shallow: true})
			}
	...
	const handelClick = useEffect(() => {
		...
		if(page === 2) {
			setPrevHref(`/category/fashion-and-style`)
		} else if (page === 1){
			setPrevHref('')
		} else {
  ...
  return(
		<>
			...
			<NextSeo
				title={props.fashion.data.attributes.seo.metaTitle}
				description={props.fashion.data.attributes.seo.metaDescription}
				canonical={props.fashion.data.attributes.seo.canonicalURL}
			/>
			<Box
				maxWidth="1216px"
				margin="0 auto"
			...
			>
				...
				position="relative"
			>
				{props.fashion.data.attributes.title}
				<Text
					as="span"
					bgColor="blue.500"
					width="20px"
			...
			<Text
				...
				lineHeight="28px"
			>
				{props.fashion.data.attributes.description}
			</Text>
			</Box>
			<Flex
				maxWidth="1216px"
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const client = new ApolloClient({
	...
	const latestPosts = await client.query({
		...
		variables: { category: 'Fashion & Style',  page: (context.query?.page && +(context.query?.page)) || 1 },
	})
